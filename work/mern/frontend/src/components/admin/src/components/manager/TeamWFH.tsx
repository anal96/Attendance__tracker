...
import { MapPin, CheckCircle, XCircle, AlertTriangle, Eye, Shield, Loader2, FileEdit, Info } from "lucide-react";
...
export function TeamWFH() {
  ...
  const fetchWFHRecords = async () => {
    try {
      setLoading(true);
      const response = await axios.get("http://localhost:5000/api/wfh/team", {
        withCredentials: true
      });
      setWfhRecords(response.data.wfhRecords || []);
    } catch (error: any) {
      console.error("Error fetching team WFH records:", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleVerify = async (wfhId: string, isVerified: boolean) => {
    try {
      ...
      alert("WFH registration verified successfully!");
    } catch (error: any) {
  ...
  const handleDeactivate = async (wfhId: string) => {
    ...
    try {
      ...
      await fetchWFHRecords();
      setShowDetailsDialog(false);
      alert("WFH registration deactivated successfully!");
    } catch (error: any) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold">Team Work From Home</h2>
        <p className="text-muted-foreground">
          View and monitor your team's work-from-home registrations and location alerts
        </p>
      </div>
        ...
        <TabsContent value="registrations" className="space-y-4">
          ...
          {loading ? (
            <Card className="p-12 text-center">
              ...
              <p className="text-muted-foreground">Loading team WFH records...</p>
            </Card>
          ) : filteredRecords.length === 0 ? (
            <Card className="p-12 text-center">
              <MapPin className="h-12 w-12 mx-auto mb-4 text-muted-foreground" />
              <p className="text-muted-foreground">No team WFH registrations found</p>
            </Card>
          ) : (
            <Card>
              <Table>
        ...
        <TabsContent value="update-requests" className="space-y-4">
          <div className="flex justify-between items-center mb-4">
            <p className="text-sm text-muted-foreground">
              View and review update requests from your team members
            </p>
            <Button
              variant="outline"
              size="sm"
          ...
          {updateRequests.length === 0 ? (
            <Card className="p-12 text-center">
              ...
              <p className="text-xs text-muted-foreground mt-2">
                Update requests from your team members will appear here
              </p>
            </Card>
          ) : (
            <Card>
        ...
        <TabsContent value="alerts" className="space-y-4">
          ...
          {filteredAlerts.length === 0 ? (
            <Card className="p-12 text-center">
              ...
              <p className="text-muted-foreground">No spoofing alerts found for your team</p>
            </Card>
          ) : (
            <Card>
              <Table>
      ...
      <Dialog open={showDetailsDialog} onOpenChange={setShowDetailsDialog}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              View team member's work-from-home registration
            </DialogDescription>
          </DialogHeader>
          {selectedRecord && (
      ...
      <Dialog open={showAlertDialog} onOpenChange={setShowAlertDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Detailed information about the location mismatch for your team member
            </DialogDescription>
          </DialogHeader>
          {selectedAlert && (
            <div className="space-y-4">
              ...
              <Alert className="bg-blue-50 border-blue-200">
                ...
                <AlertDescription className="text-blue-800">
                  An alert email has been sent to the employee, admin, and you (manager) regarding this location mismatch.
                </AlertDescription>
              </Alert>
            </div>
          )}
